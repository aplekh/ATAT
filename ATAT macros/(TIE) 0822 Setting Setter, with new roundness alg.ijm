...
if (SettingsStatus == 1) {
	...
	PrevRoundnessAlg = Table.getString("Roundness Alg", 0);
	PrevMinRoundness = Table.getString("Min Roundness", 0);
	PrevMaxRoundness = Table.getString("Max Roundness", 0);
	...
	PrevRoundnessAlg = "Yes"
	PrevMinRoundness = 0.4;
	PrevMaxRoundness = 1.00;
...
Dialog.addChoice("Use additional roundness algorithm", newArray("Yes", "No"), PrevRoundnessAlg);
...
roundnessAlgSetting = Dialog.getChoice();
...
if (roundnessAlgSetting == "Yes")
{
	Dialog.create("Roundness Settings");
	Dialog.addNumber("Minimum Roundness:", PrevMinRoundness);
	Dialog.addNumber("Maximum Roundness:", PrevMaxRoundness);
	html = "<html>" 
	+ "<h2> Roundness Algorithm Help</h2>"
	+ "This roundness algorithm will measure the radii of the particle at a number of angles. <br>"
	+" The roundness it returns is given as: <br>" 
	+ "r/(r + Q_3 - Q_1), <br>"
	+ "where r is the mean radius and Q_3 - Q_1 is the interquartile range of the radii.";
	Dialog.addHelp(html);
	Dialog.show()
	minRoundness = Dialog.getNumber();
	maxRoundness = Dialog.getNumber();
	Ext.setupRoundness("true", minRoundness, maxRoundness); 
}
...
Table.set("Roundness Alg", 0, roundnessAlgSetting);

if (roundnessAlgSetting == "Yes")
{
	Table.set("Min Roundness", 0, minRoundness);
	Table.set("Max Roundness", 0, maxRoundness);
}
