...
FibrositySetting = Table.getString("Fibrosity", 0);
...
for (z = 0; z < filecount; z++) {
	...
	runMacro("080922 Analytic Macro.ijm");
}
...
concatresultsfulldir = summarizeddatadir + lasttablecsv;
...
concatadipofulldir = summarizeddatadir + adipocsv;
...
if (FibrositySetting == "Yes") {
	...
	concatmeanfibfulldir = summarizeddatadir + meanfibcsv;
	...
	concatMedianfibfulldir = summarizeddatadir + Medianfibcsv;
	...
	concatLongestObservedfibfulldir = summarizeddatadir + LongestObservedfibcsv;
...
if (HistoBuddySetting == "Yes") {
	...
	selectWindow("Longest Observed Fibrosity Binned Results");
	LOFBRname = summarizeddatadir + inputdirname + replace(analysistime, ":", "_") + "_Longest Observed Fibrosity Binned Results" + ".csv";
	...
	selectWindow("Median Fibrosity Binned Results");
	MedFibBRname = summarizeddatadir + inputdirname + replace(analysistime, ":", "_") + "_Median Fibrosity Binned Results" + ".csv";
	...
	selectWindow("Mean Fibrosity Binned Results");
	MeanFibBRname = summarizeddatadir + inputdirname + replace(analysistime, ":", "_") + "_Mean Fibrosity Binned Results" + ".csv";
	...
	selectWindow("Adipocyte Area Binned Results");
	AdipoBRname = summarizeddatadir + inputdirname + replace(analysistime, ":", "_") + "_Adipocyte Area Binned Results" + ".csv";
