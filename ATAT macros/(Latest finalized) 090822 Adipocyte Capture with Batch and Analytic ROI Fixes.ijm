...
IJ.renameResults("AT-AT Capture Settings.csv", "Results");
run("Clear Results");
...
// attempting to solve errors on server
tempFilesArray = files;
for (i = 0; i < files.length; i++)
{
              if (!(endsWith(files[i], ".Tif") || endsWith(files[i], ".tif")))
              {
                             tempFilesArray = Array.concat(Array.slice(tempFilesArray, 0, i), Array.slice(tempFilesArray, i+1, tempFilesArray.length));
                             print(files[i] + " in ");
              }
}
if (files.length <= 0)
{
              exit("no compatible files found!");
}
...
// number of skipped images - no cells detected
numSkipped = 0;
skippedFiles = newArray();
...
for (z = 0; z < filecount; z++) {
	...
	//runMacro("051222 Adipocyte Capture with Batch Enablement and Analytic ROI Fix.ijm"));
	// ^ BAD:runMacro doesn't run from plugin; instead runs from ImageJ's "macros" folder.
	// so, we have to use a custom "MacroRunner" as part of AT-AT to be able to run them properly.
	run("MacroRunner", "051222 Adipocyte Capture with Batch Enablement and Analytic ROI Fix.ijm");
	
	/* Check if the above ran macro created a stats file. If it did, that means it was
	 * successfully executed and ROIs were found. If the stats file does not exist,
	 * no ROIs were found on the image, and hence the image needs to be skipped to avoid
	 * errors. When we skip it we also execute the necessary code to ensure everything
	 * runs smoothly, and we also let the user know that the image was skipped */
	rStatsResultsTif = rawdatadir + imagename + "_Stats.csv";
	rStatsResults = replace(rStatsResultsTif, ".tif", "");
	if (File.exists(rStatsResults) != 1)
	{
		// With the way this is set up right now, it should completely ignore bad images and do nothing else
		currentFileArray = newArray(files[z]); 
		skippedFiles = Array.concat(skippedFiles, currentFileArray);
		files = Array.deleteIndex(files, z);
		samplenames = Array.deleteIndex(samplenames, z);
		filecount--;
		numSkipped++;
		z--;
	}
	//waitForUser("Filecount: " + filecount + ". Numskipped: " + numSkipped);
}

currentsamplename = newArray(filecount + numSkipped);
currentpeakval = newArray(filecount + numSkipped);
currentpeakcount = newArray(filecount + numSkipped);
currentredstatus = newArray(filecount + numSkipped);
currentbluestatus = newArray(filecount + numSkipped);
currentgreenstatus = newArray(filecount + numSkipped);
currentanalyticarea = newArray(filecount + numSkipped);
currenttailvalue = newArray(filecount + numSkipped);
currentderivative = newArray(filecount + numSkipped);
currentscale = newArray(filecount + numSkipped);
currentfibrousarea = newArray(filecount + numSkipped);
currentweightedaverage = newArray(filecount + numSkipped);
currentmeanarea = newArray(filecount + numSkipped);
currentadipocount = newArray(filecount + numSkipped);
currentadipodensity = newArray(filecount + numSkipped);
currentmeanavgthickness = newArray(filecount + numSkipped);
currentmeanhighthickness = newArray(filecount + numSkipped);
currentmeanmedianthickness = newArray(filecount + numSkipped);
currentfibrousfraction = newArray(filecount + numSkipped);
...
/* We will append the necessary data to ...SummarizedResults.csv for all images
 * that were skipped (to indicate that they were skipped)*/
for (j = 0; j < numSkipped; j++)
{
	i = filecount;
	currentsamplename[i + j] = skippedFiles[j];
	currentpeakval[i + j] = "N/A";
	currentpeakcount[i + j] = "N/A";
	currentredstatus[i + j] = "N/A";
	currentbluestatus[i + j] = "N/A";
	currentgreenstatus[i + j] = "N/A";
	currentanalyticarea[i + j] = "N/A";
	currenttailvalue[i + j] = "N/A";
	currentderivative[i + j] = "N/A";
	currentscale[i + j] = "N/A";
	currentfibrousarea[i + j] = "N/A";
	currentweightedaverage[i + j] = "N/A";
	currentmeanarea[i + j] = "N/A";
	currentadipocount[i + j] = "N/A";
	currentadipodensity[i + j] = "N/A";
	currentmeanavgthickness[i + j] = "N/A";
	currentmeanhighthickness[i + j] = "N/A";
	currentmeanmedianthickness[i + j] = "N/A";
	currentfibrousfraction[i + j] = "N/A";
}
...
Table.rename("AT-AT Capture Settings.csv" + settingsrecordname);
