...
darkensetting = Table.getString("Darken Edges", 0);

if (darkensetting == "Yes") {
	darkenstrength = Table.get("Edge Darkening Strength", 0);
	darkensensitivity = Table.get("Edge Darkening Sensitivity", 0);
}

brightensetting = Table.getString("Brighten Images", 0);
watershedsetting = Table.getString("Watershed", 0);
...
//Also requires " 032221 right from highest thresh"
if (brightensetting == "Yes") {
	runMacro("081022 Image Brightener.ijm");
}
